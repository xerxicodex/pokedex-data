 ...
 * @method static Str[] lines(string $str, string $encoding = null)
...
class StrFacade
{
     ...
     * the last argument. For example, the following static call:
     * Str::slice('fòôbàř', 0, 3, 'UTF-8'); translates to
     * Str::create('fòôbàř', 'UTF-8')->slice(0, 3);
     * The result is not casted, so the return value may be of type Str,
     * integer, boolean, etc.
     *
     * @param string $name
    ...
    public static function __callStatic($name, $arguments)
    {
        if ( ! static::$methodArgs) {
            $strClass = new ReflectionClass(static::class);
            $methods  = $strClass->getMethods(ReflectionMethod::IS_PUBLIC);
        ...
        $strobj = Str::create($str, $encoding);

        return call_user_func_array(array($strobj, $name), $args);
    }
