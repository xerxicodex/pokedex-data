...
namespace metaunicorn\Pokedata\Common;
 ...
 *
 * This class is borrowed from the Stringify Project, with some modifications.
 * Also borrows code from Doctrine\Common\Inflector\Inflector for pluralize/singularize english words.
 */
class Str implements Countable, IteratorAggregate, ArrayAccess
{
    ...
    /**
     * Initializes a Str object and assigns both str and encoding properties
    ...
    public function camelize()
    {
        ...
        $strobj      = $this->trim()->lowerCaseFirst();
        $strobj->str = preg_replace('/^[-_]+/', '', $strobj->str);

        $strobj->str = preg_replace_callback(
            '/[-_\s]+(.)?/u',
    ...
    {
        $strobj = static::create($this->str, $this->encoding);

        if ( ! $strobj->startsWith($substring)) {
            $strobj->str = $substring . $strobj->str;
        }
    ...
    public function ensureRight($substring)
    {
        ...
        if ( ! $strobj->endsWith($substring)) {
            $strobj->str .= $substring;
        }
    ...
    public function insert($substring, $index)
    {
        ...
        $start = \mb_substr($strobj->str, 0, $index, $strobj->encoding);
        $end   = \mb_substr($strobj->str, $index, $strobj->length(),
            $strobj->encoding);

        $strobj->str = $start . $substring . $end;
    ...
    public function longestCommonSubstring($otherStr)
    {
        ...
        $encoding    = $this->encoding;
        $strobj      = static::create($this->str, $encoding);
        ...
        for ($i = 1; $i <= $strLength; $i++) {
            for ($j = 1; $j <= $otherLength; $j++) {
                $strChar   = \mb_substr($strobj->str, $i - 1, 1, $encoding);
        ...
        $strobj->str = \mb_substr($strobj->str, $end - $len, $len, $encoding);
    ...
    public function removeLeft($substring)
    {
        ...
        if ($strobj->startsWith($substring)) {
            $substringLength = \mb_strlen($substring, $strobj->encoding);

            return $strobj->substr($substringLength);
        }
    ...
    public function removeRight($substring)
    {
        ...
        if ($strobj->endsWith($substring)) {
            ...
            return $strobj->substr(0, $strobj->length() - $substringLength);
        }
    ...
    public function slugify($replacement = '-')
    {
        ...
        $strobj->str       = preg_replace($pattern, '', $strobj);

        return $strobj->toLowerCase()->delimit($replacement)
     ...
     * array of Str objects. An optional integer $limit will truncate the
     * results.
     *
    ...
    {
        $strobj   = static::create($this->trim(), $this->encoding);
        $encoding = $this->encoding;
        ...
        $strobj->str = preg_replace_callback(
            ...
            function ($match) use ($encoding, $ignore) {
                ...
                $strobj = new static($match[0], $encoding);

                return (string)$strobj->toLowerCase()->upperCaseFirst();
            },
            $strobj->str
        );
    ...
    public function truncate($length, $substring = '')
    {
        ...
        $truncated   = \mb_substr($strobj->str, 0, $length, $strobj->encoding);
        $strobj->str = $truncated . $substring;
    ...
    {
        $strobj = static::create($this->str, $this->encoding);
        $length = \mb_strlen($padStr, $strobj->encoding);
        ...
        $leftPadding  = \mb_substr(str_repeat($padStr, ceil($left / $length)), 0,
            $left, $strobj->encoding);
        $rightPadding = \mb_substr(str_repeat($padStr, ceil($right / $length)),
            0, $right, $strobj->encoding);

        $strobj->str = $leftPadding . $strobj->str . $rightPadding;
    ...
    private function supportsEncoding()
    {
        ...
        if (isset($supported[$this->encoding])) {
            ...
            throw new \RuntimeException(static::class . ' requires the ' .
